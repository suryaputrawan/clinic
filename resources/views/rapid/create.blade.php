        ...
        <div class="page-header float-right">
            <div class="page-title">
                <ol class="breadcrumb text-right">
                    ...
                    <li class="active">Input Rapid Test</li>
                </ol>
            </div>
        </div>
    </div>
  ...
  <div class="animated fadeIn">
    ...
    <div class="card">
      <div class="card-header">
        <div class="pull-left">
          <strong>Input Data</strong> Rapid Test
        </div>
      ...
      <div class="card-body card-block">
        <form action="{{ route('rapidtest.store') }}" method="post" enctype="multipart/form-data" class="form-horizontal">
          @csrf
          <div class="row form-group">
            <div class="col col-md-3"><label for="tanggal" class=" form-control-label">Tanggal Rapid Test</label></div>
            <div class="col-12 col-md-9"><input type="date" id="tanggal" name="tanggal" placeholder="Text" class="form-control @error('tanggal') is-invalid @enderror" value="{{ old('tanggal') }}" autocomplete="off">
              @error('tanggal')
            ...
            <div class="col-12 col-md-9">
              <select name="plebotomis_id" id="plebotomis" class="form-control @error('plebotomis_id') is-invalid @enderror">
                ...
                @foreach ($plebotomis as $plebo)
                  <option value="{{ $plebo->id }}" {{ old('plebotomis_id') == $plebo->id ? 'selected' : null }}>{{ $plebo->name }}</option>
                @endforeach
              </select>
              @error('plebotomis_id')
                <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="col-12 col-md-9">
              <select name="labstaff_id" id="labstaff" class="form-control @error('labstaff_id') is-invalid @enderror">
                ...
                @foreach ($labstaff as $lab)
                  <option value="{{ $lab->id }}" {{ old('labstaff_id') == $lab->id ? 'selected' : null }}>{{ $lab->name }}</option>
                @endforeach
              </select>
              @error('labstaff_id')
                <div class="invalid-feedback">{{ $message }}</div>
          ...
          <div class="row form-group">
            ...
            <div class="col-12 col-md-9">
              <select name="result" id="result" class="form-control @error('result') is-invalid @enderror">
                <option value="">Please select</option>
                <option value="Reactive IgG" {{ old('result') == "Reactive IgG" ? 'selected' : null }}>Reactive IgG</option>
                <option value="Reactive IgM" {{ old('result') == "Reactive IgM" ? 'selected' : null }}>Reactive IgM</option>
                <option value="Reactive IgG & IgM" {{ old('result') == "Reactive IgG & IgM" ? 'selected' : null }}>Reactive IgG & IgM</option>
                <option value="Non Reactive" {{ old('result') == "Non Reactive" ? 'selected' : null }}>Non Reactive</option>
              </select>
              @error('result')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
