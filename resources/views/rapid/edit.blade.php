        ...
        <div class="page-header float-right">
            <div class="page-title">
                <ol class="breadcrumb text-right">
                    ...
                    <li class="active">Edit Rapid Test</li>
                </ol>
            </div>
        </div>
    </div>
  ...
  <div class="animated fadeIn">
    ...
    <div class="card">
      <div class="card-header">
        <div class="pull-left">
          <strong>Edit Data</strong> Rapidtest
        </div>
      ...
      <div class="card-body card-block">
        <form action="{{ route('rapidtest.update', $rapid->id) }}" method="post" enctype="multipart/form-data" class="form-horizontal">
          @csrf
          <div class="row form-group">
            <div class="col col-md-3"><label for="tanggal" class=" form-control-label">Tanggal Rapid Test</label></div>
            <div class="col-12 col-md-9"><input type="date" id="tanggal" name="tanggal" placeholder="Text" class="form-control @error('tanggal') is-invalid @enderror" value="{{ old('tanggal', $rapid->tanggal) }}" autocomplete="off">
              @error('tanggal')
          ...
          <div class="row form-group">
            ...
            <div class="col-12 col-md-9"><input type="text" id="nosurat" name="nosurat" placeholder="Masukkan Nomor Surat" class="form-control @error('nosurat') is-invalid @enderror" value="{{ old('nosurat', $rapid->nosurat) }}" autocomplete="off">
              @error('nosurat')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
          ...
          <div class="row form-group">
            ...
            <div class="col-12 col-md-9"><input type="text" id="pob" name="pob" placeholder="Place Of Birth" class="form-control @error('pob') is-invalid @enderror" value="{{ old('pob', $rapid->pob) }}" autocomplete="off">
              @error('pob')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
            ...
            <div class="col-12 col-md-9">
              <select name="nationality_nationID" id="nationality" class="form-control @error('nationality_nationID') is-invalid @enderror">
                ...
                @foreach ($nationality as $nation)
                  <option value="{{ $nation->nationID }}" {{ old('nationality_nationID', $rapid->nationality_nationID) == $nation->nationID ? 'selected' : null }}>{{ $nation->nationName }}</option>
                @endforeach
              </select>
            ...
            <div class="col-12 col-md-9">
              <select name="dokter_dokterID" id="doctor" class="form-control @error('dokter_dokterID') is-invalid @enderror" value="{{ old('dokter_dokterID') }}">
                ...
                @foreach ($dokter as $dok)
                  <option value="{{ $dok->dokterID }}" {{ old('dokter_dokterID', $rapid->dokter_dokterID) == $dok->dokterID ? 'selected' : null }}>{{ $dok->doktername }}</option>
                @endforeach
              </select>
            ...
            <div class="col-12 col-md-9">
              <select name="plebotomis_id" id="plebotomis" class="form-control @error('plebotomis_id') is-invalid @enderror">
                ...
                @foreach ($plebotomis as $plebo)
                  <option value="{{ $plebo->id }}" {{ old('plebotomis_id', $rapid->plebotomis_id) == $plebo->id ? 'selected' : null }}>{{ $plebo->name }}</option>
                @endforeach
              </select>
              @error('plebotomis_id')
                <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="col-12 col-md-9">
              <select name="labstaff_id" id="labstaff" class="form-control @error('labstaff_id') is-invalid @enderror">
                ...
                @foreach ($labstaff as $lab)
                  <option value="{{ $lab->id }}" {{ old('labstaff_id', $rapid->labstaff_id) == $lab->id ? 'selected' : null }}>{{ $lab->name }}</option>
                @endforeach
              </select>
              @error('labstaff_id')
                <div class="invalid-feedback">{{ $message }}</div>
          ...
          <div class="row form-group">
            ...
            <div class="col-12 col-md-9">
              <select name="result" id="result" class="form-control @error('result') is-invalid @enderror">
                <option value="">Please select</option>
                <option value="Reactive IgG" {{ old('result', $rapid->result) == "Reactive IgG" ? 'selected' : null }}>Reactive IgG</option>
                <option value="Reactive IgM" {{ old('result', $rapid->result) == "Reactive IgM" ? 'selected' : null }}>Reactive IgM</option>
                <option value="Reactive IgG & IgM" {{ old('result', $rapid->result) == "Reactive IgG & IgM" ? 'selected' : null }}>Reactive IgG & IgM</option>
                <option value="Non Reactive" {{ old('result', $rapid->result) == "Non Reactive" ? 'selected' : null }}>Non Reactive</option>
              </select>
              @error('result')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
          ...
          <div class="row form-group">
            ...
            <div class="col-12 col-md-9"><textarea name="remarks" id="remarks" rows="5" placeholder="" class="form-control" autocomplete="off">{{ old('remarks', $rapid->remarks) }}</textarea></div>
          </div>
          <div class="row form-group">
