        ...
        <div class="page-header float-right">
            <div class="page-title">
                <ol class="breadcrumb text-right">
                    ...
                    <li class="active">Input Serology Test</li>
                </ol>
            </div>
        </div>
    </div>
  ...
  <div class="animated fadeIn">
    ...
    <div class="card">
      <div class="card-header">
        <div class="pull-left">
          <strong>Input Data</strong> Serology Test
        </div>
      ...
      <div class="card-body card-block">
        <form action="{{ route('serology.store') }}" method="post" enctype="multipart/form-data" class="form-horizontal">
          @csrf
            ...
            <div class="col-12 col-md-9">
              <select name="laboratorium_id" id="labstaff" class="form-control @error('laboratorium_id') is-invalid @enderror">
                ...
                @foreach ($laboratorium as $lab)
                  <option value="{{ $lab->id }}" {{ old('laboratorium_id') == $lab->id ? 'selected' : null }}>{{ $lab->name }}</option>
                @endforeach
              </select>
          ...
          <div class="row form-group">
            ...
            <div class="col-12 col-md-9">
              <select name="result" id="result" class="form-control @error('result') is-invalid @enderror">
                <option value="">Please select</option>
                <option value="Reactive" {{ old('result') == "Positif" ? 'selected' : null }}>Reactive</option>
                <option value="Non Reactive" {{ old('result') == "Negative" ? 'selected' : null }}>Non Reactive</option>
              </select>
              @error('result')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
